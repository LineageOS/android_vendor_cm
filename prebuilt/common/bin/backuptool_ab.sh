...
cp -f /postinstall/bin/backuptool_ab.functions /postinstall/tmp/backuptool.functions
...
# Restore /postinstall/addon.d from /postinstall/tmp/addon.d
restore_addon_d() {
  if [ -d /postinstall/tmp/addon.d/ ]; then
    mkdir -p /postinstall/addon.d/
    cp -a /postinstall/tmp/addon.d/* /postinstall/addon.d/
...
if [ -d /postinstall/tmp/addon.d/ ]; then
  for script in $(find /postinstall/tmp/addon.d/ -name '*.sh' |sort -n); do
    ...
    # as a pseudo-/tmp dir
    sed -i 's|. /tmp/backuptool.functions|. /tmp/backuptool.functions|g' $script
    $script $1
  done
fi
}
