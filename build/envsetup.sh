...
function breakfast()
{
    ...
    if [ $# -eq 0 ]; then
        ...
        if echo $target | grep -q "lineage_"; then
            target=$(echo $target | sed "s|lineage_||g")
        elif echo $target | grep -q "cm_"; then
            target=$(echo $target | sed "s|cm_||g")
        fi
        ...
        if [ $? -eq 0 ]; then
            ...
            # Remove variant for check_product
            ...
            if ! check_product lineage_$target && check_product cm_$target; then
                ...
                lunch cm_$target_variant
            else
                lunch lineage_$target_variant
            fi
        else
