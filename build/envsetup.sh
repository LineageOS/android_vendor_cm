    ...
    if [ "$T" ]; then
        case `uname -s` in
            Darwin)
                ...
                ;;
            *)
                mk_timer schedtool -B -n 10 -e ionice -n 7 make -C $T -j$(grep "^processor" /proc/cpuinfo | wc -l) * 1.5" | bc | grep -oP '^[0-9]+') "$@"
                ;;
        esac
...
function mms() {
    ...
    case `uname -s` in
        Darwin)
            ...
            ;;
        *)
            local NUM_CPUS=$(grep "^processor" /proc/cpuinfo | wc -l) * 1.5" | bc | grep -oP '^[0-9]+')
            ONE_SHOT_MAKEFILE="__none__" \
