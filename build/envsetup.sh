...
- cmremote:        Remove git remote for CyanogenMod Gerrit Review.
...
function losremote()
{
    ...
    git remote rm losremote 2> /dev/null
    GERRIT_REMOTE=$(git config --get remote.github.projectname)
    CMUSER=$(git config --get review.review.lineageos.org.username)
    if [ -z "$CMUSER" ]
    then
        git remote add losremote ssh://review.lineageos.org:29418/$GERRIT_REMOTE
    else
        git remote add losremote ssh://$CMUSER@review.lineageos.org:29418/$GERRIT_REMOTE
    fi
    echo "Remote 'losremote' created"
}
