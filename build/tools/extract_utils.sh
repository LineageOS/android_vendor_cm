...
function write_header() {
    ...
    if [ $YEAR -eq 2017 ]; then
        printf "#           (C) 2017 The LineageOS Project\n" >> $1
    else
        printf "#           (C) 2017-$YEAR The LineageOS Project\n" >> $1
    fi
...
function oat2dex() {
    ...
    if [ -z "$BAKSMALIJAR" ] || [ -z "$SMALIJAR" ]; then
        export BAKSMALIJAR="$ROM_ROOT"/vendor/cm/build/tools/smali/baksmali.jar
        export SMALIJAR="$ROM_ROOT"/vendor/cm/build/tools/smali/smali.jar
    fi
    ...
    if grep "classes.dex" "$ROM_TARGET" >/dev/null; then
...
function extract() {
    ...
    for (( i=1; i<COUNT+1; i++ )); do
        ...
        if [ "$SRC" = "adb" ]; then
            ...
            if [ -f "$SRC/$FILE" ]; then
                cp "$SRC/$FILE" "$DEST"
            # if file does not exist try ROM target
