...
int main()
{
    ...
    for (tinyxml2::XMLElement *package = root->FirstChildElement("package"); package != NULL;
        ...
        if (key != NULL) {
            ...
            keysMap[certIndex] = key;
        }
        const char *packageName = package->Attribute("name");
        if (!packageFound && !std::strcmp(packageName, RELEASE_PACKAGE)) {
            int certIndex;
            cert->QueryIntAttribute("index", &certIndex);
            packageCertIndex = certIndex;
            packageFound = true;
        }
        if (packageFound && keysMap.count(packageCertIndex)) {
            if (keysMap[packageCertIndex] != releaseKey) {
